...
export const headscaleApi = {
  ...
  users: {
    ...
    create: (body: { name: string; displayName?: string; email?: string }) =>
      api<{ user: import('@/lib/types').User }>('/user', {
        method: 'POST',
  ...
  nodes: {
    ...
    expire: (id: string) =>
      api<{ node: import('@/lib/types').Node }>(
        `/node/${encodeURIComponent(id)}/expire?disableExpiry=false`,
        { method: 'POST' }
      ),
    rename: (id: string, name: string) =>
      api<{ node: import('@/lib/types').Node }>(
        `/node/${encodeURIComponent(id)}/rename/${encodeURIComponent(name)}`,
        { method: 'POST' }
      ),
    setTags: (id: string, tags: string[]) =>
      api<{ node: import('@/lib/types').Node }>(`/node/${encodeURIComponent(id)}/tags`, {
        method: 'POST',
        body: JSON.stringify({ tags }),
      }),
    setApprovedRoutes: (id: string, routes: string[]) =>
      api<{ node: import('@/lib/types').Node }>(`/node/${encodeURIComponent(id)}/approve_routes`, {
        method: 'POST',
  ...
  apiKeys: {
    ...
    expire: (prefix: string) =>
      api<void>('/apikey/expire', { method: 'POST', body: JSON.stringify({ prefix }) }),
    delete: (prefix: string, id: string) =>
      api<void>(`/apikey/${encodeURIComponent(prefix)}?id=${encodeURIComponent(id)}`, {
        method: 'DELETE',
      }),
  },
  preAuthKeys: {
    ...
    create: (body: Record<string, unknown>) =>
      api<{ preAuthKey: import('@/lib/types').PreAuthKey }>('/preauthkey', {
        method: 'POST',
        body: JSON.stringify(body),
      }),
    expire: (id: string) =>
      api<void>('/preauthkey/expire', { method: 'POST', body: JSON.stringify({ id }) }),
  },
  policy: {
    get: () => api<{ policy: string; updatedAt: string }>('/policy'),
    set: (policy: string) =>
      api<{ policy: string; updatedAt: string }>('/policy', {
        method: 'PUT',
