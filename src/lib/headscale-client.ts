...
export function createHeadscaleClient(baseUrl: string, apiKey: string) {
  ...
  return {
    ...
    preAuthKeys: {
      list: () => request<{ preAuthKeys: PreAuthKey[] }>('/api/v1/preauthkey'),
      create: (body: {
        user: string;
        reusable?: boolean;
        ephemeral?: boolean;
    ...
    apiKeys: {
      list: () => request<{ apiKeys: ApiKey[] }>('/api/v1/apikey'),
      ...
      delete: (prefix: string, id?: string) =>
        request<void>(
          `/api/v1/apikey/${encodeURIComponent(prefix)}${id ? `?id=${encodeURIComponent(id)}` : ''}`,
          { method: 'DELETE' }
        ),
    },

    policy: {
      get: () => request<{ policy: string; updatedAt: string }>('/api/v1/policy'),
      set: (policy: string) =>
